...
class GAP_net(nn.Module):
    ...
    def forward(self, y, Phi, Phi_s):
        ...
        ### stage 4
        yb = A(x,Phi)
        x = x + At(torch.div(y4-yb,Phi_s),Phi)
        x = shift_back(x)
        x = self.unet4(x)
        x = self.maxpool_x(x)
        Phi = self.maxpool_phi(Phi)
        y5 = self.maxpool_y(y4)
        Phi_s = torch.sum(Phi, dim=1)
        Phi_s[Phi_s==0] = 1
        x = shift(x)
        ...
        ### stage 5
        yb = A(x,Phi)
        x = x + At(torch.div(y5-yb,Phi_s),Phi)
        x = shift_back(x)
        x = self.unet5(x)
        x = self.upsample4_x(x)          
        Phi = self.upsample4_phi(Phi)
        ...
        x = shift_back(x)
        x = self.unet6(x)
        x = self.upsample3_x(x)          
        Phi = self.upsample3_phi(Phi)
